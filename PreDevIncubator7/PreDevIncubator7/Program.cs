...
namespace PreDevIncubator7
{
    ...
    public class Garage
    {
        ...
        private Vehicle[] items = new Vehicle[MaxSize];
        ...
        {
            currentIndex++;
            items[currentIndex] = Vehicle;
            Console.WriteLine($"Auto{Vehicle.VehicleId} заехало в гараж");
            if(currentIndex==MaxSize-1)
                Console.WriteLine("Гараж заполнен");
        }
        public Vehicle Pop()
        {           
            Vehicle Vehicle = items[currentIndex];
            items[currentIndex] = null;
            currentIndex--;
            Console.WriteLine($"Auto{Vehicle.VehicleId} выехало из гаража");
            return Vehicle;
        }
        public Vehicle Peek()
        {
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for(int i =1; i<16;i++)
            {
                var vehicle = new Vehicle { VehicleId = i };
                garage.Push(vehicle);
            }
            for (int i = 1; i < 16; i++)
            {
